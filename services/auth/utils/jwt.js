...
class JWTManager {
  ...
  static generateAccessToken(user, scope = []) {
    ...
    // Only add audience if CLIENT_ID is defined
    if (process.env.CLIENT_ID) {
      options.audience = process.env.CLIENT_ID;
    }
  ...
  static generateIDToken(user, nonce = null) {
    ...
    return jwt.sign(payload, process.env.ID_TOKEN_SECRET || process.env.ACCESS_TOKEN_SECRET, options);
  }
  ...
  static verifyIDToken(token) {
    try {
      ...
      return jwt.verify(token, process.env.ID_TOKEN_SECRET || process.env.ACCESS_TOKEN_SECRET, options);
    } catch (error) {
